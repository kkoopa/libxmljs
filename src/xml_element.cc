...
NAN_METHOD(XmlElement::New) {
  ...
  args.Holder()->Set(NanSymbol("document"), args[0]);
...
v8::Local<v8::Value>
XmlElement::get_name() {
    if(xml_obj->name) return NanNew<v8::String>((const char*)xml_obj->name);
    else return NanNew(NanUndefined());
}
